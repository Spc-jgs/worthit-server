...
> 状态：`IMPLEMENTED`。2026-08-09 用户确认完整方案、新增必要业务表与上级权威文档
> 更新；PRD V0.16.0、架构 V0.3.23、接口 V0.2.6、数据库 V0.3.9、技术门禁
> V0.3.6 与产品验收 V0.5.6 已将该方案纳入现行基线。实现与验证证据见
> [M3 账号注销交付记录](../tasks/2026-08-09-m3-account-cancellation-delivery.md)。
...
完整设计已确认，上级独立文档工作区已建立新的 PRD/架构/接口/数据库/技术门禁/产品
验收版本、更新定稿索引并完成 DOCX 渲染核验。上级文档属于独立、当前未跟踪的
工作区，不混入后端仓库提交；其后续纳入版本控制方式由上级项目单独处理。
...
最终 HEAD 核验。本设计提案独立合并后，业务实现从最新 `main` 另开 feature branch 交付。
...
4. 本轮业务代码范围是账号注销；同步隐私说明，并单独建立含“注销数据不复活”的备份恢复
   子设计与可执行验收。
5. 允许修改当前未跟踪的上级权威文档；本轮已完成升版与旧版归档。
