...
- `TODO-BIZ-001`、`TODO-BIZ-002`、`TODO-P1-001` 和 `TODO-P1-002` 均已完成，保留
  下文作为历史问题与验收依据，不再视为当前待办。
- `TODO-P1-003` 仍是未完成的独立技术债；当前 M3 后端收口任务明确不处理 Flyway
  兼容治理，不得把现有 MySQL 8.4 迁移成功误写成该兼容问题已经消失。
- `TODO-DOC-001` 已由 2026-08-04 定稿的 PRD V0.15.9、架构 V0.3.22、接口 V0.2.5、
  数据库模型 V0.3.8 及定稿索引完成同步。
...
未完成，保留为独立 P1 技术债。当前 M3 后端收口任务按用户要求明确排除 Flyway
兼容治理；迁移测试继续执行并如实保留兼容警告，不在本任务中升级或覆盖版本。
...
已完成。现行 PRD V0.15.9 已冻结密码登录适用端和统一 Bearer 登录态；架构 V0.3.22、
接口 V0.2.5、数据库模型 V0.3.8 与定稿索引已经同步账号密码登录、凭据边界、错误语义
及 Auth V2 迁移事实。下述“现状”仅保留为 2026-07-26 的历史记录。
